...
export const getAllPaymentsRestVersion = () => {
    ...
    const paymentsPromise = fetch("http://localhost:8080/api/payment", {method: "GET", headers});
...
export const getAllPaymentsAxiosVersion = () => {
    ...
    const paymentsPromise = axios({url, method: "GET", headers : {'Accept' : 'application/json'}});
...
export const getAllPaymentsRestExample = () => {
    ...
    //const paymentsPromise = fetch("http://localhost:8080/api/payment", {method: "GET", headers: headers});
    paymentsPromise.then(
        (response) => {
            if (response.ok) {
                ...
                console.log("something went wrong - server responded with", response.status, response.text);
            }
        }
    ).catch (
        (error) => {
